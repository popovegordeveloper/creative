...
class ProductService
{
    ...
    public function saveProduct($request, $user)
    {
        ...
        try { mkdir(public_path($dir_name)); } catch (\Exception $exception) { \Log::error($exception->getMessage()); }
        $product_data = $request->except(['color']);
        $photos = [];
    ...
    public function getSimilarProducts(Product $product)
    {
        ...
        return Product::active()->checked()->whereIn('category_id', $categories->pluck('id')->toArray())->inRandomOrder()->limit(4)->get();
    }
