<div class="lk__content @if($section == 'purchases') lk__content_active @endif">
    <div class="my-items">
        @if($user->orders->count())
            <div>
                <h3 class="my-items-empty__title">Мои покупки</h3>
            ...
            <div style="background: #e9e9e9; padding: 20px 35px; display: flex; justify-content: center">
                <div style="margin-right: 45px;">
                    <p style="font-weight: 500;">Купленно товаров на сумму:</p>
                    <h3 style="text-align: center;">{{ $user->purchased_sum }} ₽</h3>
                </div>
                <div>
                    <p style="font-weight: 500;">Купленно товаров:</p>
                    <h3 style="text-align: center;">{{ $user->purchased_count }}</h3>
                </div>
            </div>
            ...
            <div class="orders-body orders-body1">
                @if($user->currently_orders->count())
                    @include('blocks.orders', ['orders' => $user->currently_orders])
                @else
                    <p style="text-align: center;">У вас нет текущих заказов</p>
                @endif
            </div>
            <div class="orders-body orders-body2" style="display: none">
                @if($user->canceled_orders->count())
                    @include('blocks.orders', ['orders' => $user->canceled_orders])
                @else
                    <p style="text-align: center;">У вас нет отмененных заказов</p>
                @endif
            </div>
            <div class="orders-body orders-body3" style="display: none">
                @if($user->completed_orders->count())
                    @include('blocks.orders', ['orders' => $user->completed_orders])
                @else
        ...
        @else
            <div class="my-items-empty">
                ...
                <h3 class="my-items-empty__title">Мои покупки</h3>
                <span class="my-items-empty__subtitle">Пока нет ни одного купленного товара</span>
            </div>
        @endif
    </div>
</div>
